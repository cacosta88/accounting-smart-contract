...
contract YourContract {
    ...
    event CapitalAdjustmentVoted(address indexed owner, uint256 proposalID, uint256 voteWeight);
    ...
    function voteForCapitalProposal(
		...
		if (owners[msg.sender].ownershipPercentage < voteWeight) revert InsufficientOwnershipPercentage();
    ...
    function voteForClosePeriodProposal(uint256 proposalID, uint256 voteWeight) external onlyOwners {
    ...
    function executeCloseAccountingPeriod(uint256 proposalID) external onlyOwners {
        ...
        if (distributableIncome > 0) {
            for (uint i = 0; i < ownerAddresses.length; i++) {
                address currentOwnerAddress = ownerAddresses[i];
                uint256 ownershipPercentage = owners[currentOwnerAddress].ownershipPercentage;
		...
		for (uint256 i = 0; i < closePeriodProposalIDs.length; i++) {
			if (closePeriodProposalIDs[i] == proposalID) {
				...
				for (uint256 j = i; j < closePeriodProposalIDs.length - 1; j++) {
